...
WHAT IS MEASURED
  1. HOW MUCH OF A BETA ESTIMATE IS SIGNAL. var(observed beta) across names
     ...
     computable from each regression's own standard error, so the reliability of
  ...
  4. THE PRACTICAL CONSEQUENCE. The rate-beta decile spread measured in
     ...
     actually delivers, and it bounds what rate_foresight.py's -9.35% could ever
...
forward-looking labels, house rule 3 applies and the return spread is decomposed
...
non-overlapping observations of the forward period. Cross-sectional correlations
...
DDOF = 0                    # population moments throughout, so that the
                            ...
                            # mixing them silently biases the residual variance.
...
OUT_CSV = "rate_persistence_by_month.csv"
...
def two_factor_betas(R: pd.DataFrame, rs: pd.Series, rt: pd.Series,
    ...
    univariate betas double-counts. The 2x2 normal equations have a closed form,
    which keeps this as cheap as the univariate case.
    """
...
def build_panel() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    d = d.merge(at_dates(bs, dates, "b2_spy"), on=["date", "symbol"], how="left")
    d = d.merge(at_dates(bt, dates, "b2_tlt"), on=["date", "symbol"], how="left")
    del bs, bt
...
def section_reliability(rel: pd.DataFrame) -> dict[int, float]:
    ...
    say("=" * 78)
    say("  var(estimate) across names = var(true beta) + var(estimation noise).")
    ...
    for w, g in rel.groupby("window"):
        ...
        say(f"  {lab:<12}{np.sqrt(cs):>11.3f}{np.sqrt(nz):>12.3f}"
            f"{r:>14.2f}")
    say("")
    say("  sd(beta) is the cross-sectional spread of the estimate; sd(noise) is")
    say("  the typical standard error of one name's regression.")
    return out
...
def section_persistence(d: pd.DataFrame,
    ...
    say("  0.50 a decile sort delivers half the exposure spread it advertises.")
...
def section_by_year(d: pd.DataFrame) -> pd.DataFrame:
    say("")
    say("=" * 78)
    say(f"  3. IS THE PERSISTENCE STABLE? trailing {HEADLINE_WINDOW}d, BY YEAR")
    say("=" * 78)
...
def section_capture(d: pd.DataFrame, reli: dict[int, float],
    ...
    say("  much of the available duration spread a monthly trailing sort buys.")
    say("")
    say("  ONE CORRECTION IS NEEDED AND IT MATTERS. Sorting on rb_fwd and then")
    say("  measuring rb_fwd capitalises that estimate's own noise, so the naive")
    say(f"  ceiling is too high. A 63-day beta is only "
        f"{reli.get(-FWD_DAYS, float('nan')):.2f} signal, and the spread in the")
    say("  TRUE forward beta that a perfect sort could reach is the delivered")
    ...
    say("  and independent of the trailing sort, so decile means of rb_fwd are")
    say("  unbiased for decile means of the true forward beta.")
    say("")
    ...
    say("  the noise in the ceiling; both are reported so neither can be cherry")
    say("  picked later.")
    say("")
    say(f"  ADVERTISED vs DELIVERED. The sort looks like it buys {adv:+.3f} of")
    say(f"  trailing-beta spread; {adv_true:+.3f} of that is true beta and the")
    ...
    say(f"  {early.rb_fwd_tr.mean():+.3f} to {late.rb_fwd_tr.mean():+.3f}. The "
        "deterioration lands squarely on the")
    ...
    say("")
    say("  RETURN SPREAD, SPLIT BY REGIME. Pooling the two regimes cancels the")
    say("  sign, so the return version of this table is only meaningful within")
    say("  a regime.")
    say("")
    ...
    say("  " + "-" * 59)
    ret: dict[str, float] = {}
    for lab, mask in (("rates UP", d.tlt_fwd < 0), ("rates DOWN", d.tlt_fwd >= 0)):
        sub = d[mask]
    ...
    m.to_csv(os.path.join(LONG, OUT_CSV), index=False)
...
def section_decomposition(d: pd.DataFrame) -> dict[str, float]:
    say("")
    say("=" * 78)
    say("  5. HOUSE RULE 3: EXPOSURE x REALISED FACTOR RETURN, OR SOMETHING ELSE?")
    say("=" * 78)
    say("  rate_foresight.py's spread is reproduced here in the rates-UP months")
    say("  it reported, then split into what two-factor exposures multiplied by")
    say("  the realised SPY and TLT moves already account for. The attribution")
    say("  uses TRAILING two-factor betas, which is what was knowable, and then")
    say("  FORWARD-REALISED ones, which is what actually described the window.")
    say("")
    ...
    for lab, sub in (("rates UP", up), ("rates DOWN", dn)):
        ...
        if s.empty:
            continue
        fac = sub.groupby("date")[["tlt_fwd", "spy_fwd"]].first().reindex(s.date)
...
def section_long_only(d: pd.DataFrame) -> dict[str, float]:
    ...
    say("  rate-beta decile, held long. Its demeaned return is the excess over")
    say("  an equal-weight basket of the same universe, which itself compounded")
...
def main() -> None:
    ...
    say(f"  {len(ok):,} rows have both a trailing {HEADLINE_WINDOW}d beta and a"
        f" realised forward beta")
